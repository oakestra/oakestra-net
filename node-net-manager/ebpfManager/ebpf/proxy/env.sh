...
ip addr add 10.30.0.3/16 dev $BRIDGE
...
ip netns exec $NS1 ip addr add 10.30.0.1/16 dev veth-$NS1-peer
ip netns exec $NS2 ip addr add 10.30.0.2/16 dev veth-$NS2-peer
...
ip netns exec $NS1 ip route add default via 10.30.0.3
ip netns exec $NS2 ip route add default via 10.30.0.3
