...
MONGO_ADDR_GATEWAYS = (
    "mongodb://" + str(MONGO_URL) + ":" + str(MONGO_PORT) + "/gateways"
)
...
def mongo_init(flask_app):
    global app
    global mongo_nodes, mongo_jobs, mongo_gateways, mongo_gateway_nodes
...
def mongo_find_node_by_id_and_update_subnetwork(node_id, addr, addr_v6):
    ...
    app.logger.info(
        "MONGODB - update subnetwork of worker node {0} ...".format(node_id)
    )

    mongo_nodes.db.nodes.find_one_and_update(
        {"_id": ObjectId(node_id)},
        {"$set": {"node_subnet": addr, "node_subnet_v6": addr_v6}},
        upsert=True,
    )
...
def mongo_insert_job(job):
    ...
    if new_job.get("instance_list") is None:
        jobs.find_one_and_update(
            {"job_name": job["job_name"]}, {"$set": {"instance_list": []}}
        )
...
def mongo_update_job(job):
    ...
    current_job = mongo_jobs.db.jobs.find_one({"job_name": job.get("job_name")})
...
def mongo_get_service_ips_by_jobname(name):
    return mongo_jobs.db.jobs.find_one({"job_name": name}, {"service_ip_list": 1})
...
def mongo_update_gateway_job_namespace(gateway_id, ipv4, ipv6):
    global mongo_jobs
    jobs = mongo_jobs.db.jobs
    ...
    jobs.find_one_and_update(
        ...
        {
            "$set": {
                "instance_list.0.namespace_ip": ipv4,
                "instance_list.0.namespace_ip_v6": ipv6,
            }
        },
    )
