...
from interfaces.mongodb import closest
from typing import List

# TODO: implement closest strategy
class IPv6ClosestStrategy(IPv6InstanceStrategy):
    """IPv6 closest address allocation strategy"""
    ...
    def validate_custom_address(self, address: str, job_name: str) -> bool:
        ...
        # Check for underutilized IP range (fdff:1000::/21)
        if address_arr[0] != "fdff" or address_arr[1][0:2] != "10":
            raise Exception("Closest IP address must be in the subnet fdff:1000::/21")

        job = requests.mongo_find_job_by_ip(address)
        if job is not None and job['job_name'] != job_name:
            raise Exception("Closest IPv6 address already used by another service")
        
        return True

    def get_next_address(self) -> str:
        with self.lock:
            addr = closest.mongo_get_closest_address_from_cache_v6()
            while addr is None:
                addr = closest.mongo_get_next_closest_ip_v6()
                next_addr = self._increase_address(addr)
                closest.mongo_update_next_closest_ip_v6(next_addr)
    ...
    def clear_address(self, address: str) -> None:
        ...
        with self.lock:
            next_addr = closest.mongo_get_next_closest_ip_v6()
            ...
            closest.mongo_free_closest_address_to_cache_v6(addr)
    ...
    def _increase_address(self, addr: List[int]) -> List[int]:
        ...
        try:
            ...
            # Verify the address is still in fdff:1000::/21 subnet
            # First 21 bits should be: fdff:1000
            if new_addr[0] != 253 or new_addr[1] != 255 or new_addr[2] != 16:
                raise OverflowError("Address would be outside fdff:1000::/21 subnet")
                
            return new_addr
        except OverflowError:
            raise RuntimeError("Exhausted Closest IPv6 address space")
