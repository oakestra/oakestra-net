...
from interfaces.mongodb import underutilized
from typing import List


class IPv6UnderutilizedStrategy(IPv6InstanceStrategy):
    """IPv6 underutilized address allocation strategy"""
    ...
    def validate_custom_address(self, address: str, job_name: str) -> bool:
        ...
        # Check for underutilized IP range (fdff:3000::/21)
        if address_arr[0] != "fdff" or address_arr[1][0:2] != "30":
            raise Exception("Underutilized IP address must be in the subnet fdff:3000::/21")

        job = requests.mongo_find_job_by_ip(address)
        if job is not None and job['job_name'] != job_name:
            raise Exception("Underutilized IPv6 address already used by another service")
        
        return True

    def get_next_address(self) -> str:
        with self.lock:
            addr = underutilized.mongo_get_underutilized_address_from_cache_v6()
            while addr is None:
                addr = underutilized.mongo_get_next_underutilized_ip_v6()
                next_addr = self._increase_address(addr)
                underutilized.mongo_update_next_underutilized_ip_v6(next_addr)
    ...
    def clear_address(self, address: str) -> None:
        ...
        with self.lock:
            next_addr = underutilized.mongo_get_next_underutilized_ip_v6()
            ...
            underutilized.mongo_free_underutilized_address_to_cache_v6(addr)
    ...
    def _increase_address(self, addr: List[int]) -> List[int]:
        ...
        try:
            ...
            # Verify the address is still in fdff:3000::/21 subnet
            # First 21 bits should be: fdff:3000
            if new_addr[0] != 253 or new_addr[1] != 255 or new_addr[2] != 48:
                raise OverflowError("Address would be outside fdff:3000::/21 subnet")
                
            return new_addr
        except OverflowError:
            raise RuntimeError("Exhausted Underutilized IPv6 address space")
